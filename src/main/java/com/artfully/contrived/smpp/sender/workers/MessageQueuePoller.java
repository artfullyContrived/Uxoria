 ...
 * SMPPTOSend poller.
 * <p>
 * This class polls the SMPPTosend table every so often for messages for is smppid. It then puts them in a priority queue where consumers pick from for onward sending
 */
public class MessageQueuePoller implements Runnable {
	...
	try {
		...
		message = new ShortMessage.Builder(smpp.getSession())
			.dataCoding(
				new GeneralDataCoding(smpp.getDataEncoding()))
			.destAddrNpi(
			...
			// get a way to say if u want a delivery report in mesagequeue
			.registeredDelivery(
		...
		consumerService.submit(new MessageSender(message, limiter));
