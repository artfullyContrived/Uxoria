...
public class UxoriaUtils {
  ...
  public static ContentItem getContentItem(MyDeliverSM deliverSm2) {
    ...
    String keyword = message.indexOf(' ') > 0 ? message.substring(0, message.indexOf(' ')) : message;
    Connection conn = null;
    try {
      conn = dbUtils.getConnection();
      ...
      if (rs.next()) {
        ...
        contentItem.setId(rs.getInt("Id"));
        contentItem.setHeadText(rs.getString("headText"));
        contentItem.setContentURL(rs.getString("contentURL"));
        contentItem.setTailText(rs.getString("tailText"));
        contentItem.setKeyword(rs.getString("keyword"));
      }
    } catch (SQLException e) {
      logger.error(e, e);
    } finally {
    ...
    logger.debug("Content Element " + contentItem);
    return contentItem;
  }
